extends Node
#keybind code, ripped directly from my ogpc game 
...
func makeDefaultKeybinds()->void:
	if(!FileAccess.file_exists("user://keybinds.json")):
		var parse:JSON = JSON.new()
		var defaultkeybinds = FileAccess.open("res://defaultConfig/keybinds.json",FileAccess.READ)
...
func reloadKeybinds():
	if(!FileAccess.file_exists("user://keybinds.json")):
		...
		var res = FileAccess.open("res://defaultConfig/keybinds.json",FileAccess.READ)
